...
{
    public partial class ConnectCitizensTopUp : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                Variable_email = Session["email"].ToString();
                LblEmail.Text = Session["email"].ToString();
                LblUserID.Text = Session["userID"].ToString();
                txtDepositorEmail.Text = LblUserID.Text;
                LblRefID.Text = Session["refID"].ToString();
                ConnectRecharge cre = new ConnectRecharge();
                cre.UserId = Session["userID"].ToString();
                cre.ConnectCitizenCheckFundCreditBalance();

                cre.CitizenStatus();
                LblNickName.Text = cre.nickname;
                int refer = int.Parse(cre.Referrer);
                if(refer >= 10)
                {
                    LblClientStatus.Text += ", **Ambassador";
                }
                if(cre.vendor == 1)
                {
                    LblClientStatus.Text += ", ***Vendor";
                }

                LblWalletbalance.Text = "#" + cre.amount.ToString();
                //Get ORP
                cre.ConnectCitizenCheckEarningsBalance();
                LblEarningsBalance.Text = "#" + cre.credit.ToString();
        ...
        private DataTable GetDataTrail()
        {
            ...
            cre.ConnectCitizensGetDataTrail();
            ...
            if (amount >= 50)
            {
                if (ddlORPayOptionVTU.Text == "Wallet")
                {
                    ...
                    if (cre.credit >= amount)
                    {
                        ...
                        else if (cre.ResponseMsg == null || cre.ResponseMsg != "SUCCESS!")
                        {
                            cre.UserId = LblUserID.Text;
            ...
            if (amount >= 50)
            {
                if (ddlORPayOptionPIN.Text == "Wallet")
                {
                    ...
                    if (cre.credit >= amount)
                    {
                        ...
                        {
                            cre.UserId = LblUserID.Text;
                            cre.note = cre.PIN + " recharge PIN created.";
                            cre.ConnectCitizensTrail();
            ...
            if (txtPayStackAmount.Text != "")
            {
                ...
                if (amount >= 100)
                {
                    ...
                    if (cre.ResponseMsg == "0")
                    {
                        ...
                        cre.PayStackAmount = int.Parse(txtPayStackAmount.Text);
                        cre.CitizenRechargePayStackTrack();
                    ...
                    else if (cre.ResponseMsg == "1")
                    {
                        ...
                        if (cre1.ResponseMsg == "0")
                        {
                            ...
                            cre1.CitizenRechargePayStackTrack();
        ...
        protected void BtnSubAccountSignUp_Click(object sender, EventArgs e)
        {
            Model.ConnectRecharge cre = new Model.ConnectRecharge();
            cre.UserId = LblUserID.Text;
            cre.CheckBeforeSubAccount();
            if (cre.ResponseMsg == "0")
            {
                ...
                if (cre.ResponseMsg.Contains("Success"))
                {
                    cre.note = "Sub VTU-Account created with Program ID: " + cre.programID;
                    cre.ConnectCitizensTrail();
                    Session["AlertMessage"] = cre.ResponseMsg;
                    Response.Redirect("success.aspx");
                }
        ...
        protected void BtnVendor_Click(object sender, EventArgs e)
        {
            Model.ConnectRecharge cre = new Model.ConnectRecharge();
            cre.UserId = LblUserID.Text;
            cre.CheckConnectCitizenFundWalletBalance();
            if(cre.amount >= 5000)
            {
                cre.ConnectCitizenVendorUpgrade();
                if(cre.ResponseMsg.Contains("Insufficient"))
                {
                    Session["AlertMessage"] = cre.ResponseMsg;
                    Response.Redirect("error.aspx");
                }
            ...
            {
                Session["AlertMessage"] = "Insufficient Fund Wallet Balance. Kindly fund your wallet using any of our payment channels.";
                Response.Redirect("error.aspx");
            }
        }
